...
const VaultEarnings = () => {
			...
			try {
				...
				setError(
					err instanceof Error
						? err.message
						: "Failed to fetch vault earnings"
				);
	...
	const columns: ColumnDef<VaultData>[] = [
		...
		},
		{
			...
			cell: ({ row }) => (
				<div className="text-white text-sm font-semibold text-right">
					{row.original.profitPercent > 0 ? "+" : ""}
					{row.original.profitPercent.toFixed(2)}%
				</div>
			),
		},
		{
			...
			cell: ({ row }) => (
				...
				>
					{row.original.profit > 0 ? "+" : ""}
					{row.original.profit > 0 || row.original.profit < 0
		...
		},
		{
			...
			cell: ({ row }) => (
				<Link
					href={`/asset-vault/${row.original.vault_id}`}
