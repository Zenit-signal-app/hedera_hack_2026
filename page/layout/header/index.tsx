...
import ChainSelectorPopup from "@/components/wallet/ChainSelectorPopup";
import { listNavigators, CHAIN_DEFINITIONS } from "@/lib/constant";
				...
				<div className="flex items-center lg:px-3 lg:py-1.5 px-2 py-1.5 rounded-full  border border-dark-gray-700 bg-dark-gray-900">
					<ChainSelectorPopup>
						<div className="flex items-center gap-x-2 h-7">
							{activeChainDef ? (
								<Image
									src={activeChainDef.logo}
									width={24}
									height={24}
									alt={activeChainDef.name}
									className="lg:w-6 lg:h-6 w-5 h-5 rounded-full"
								/>
							) : (
								<span className="lg:w-6 lg:h-6 w-5 h-5 rounded-full bg-dark-gray-700 flex items-center justify-center text-white text-[10px] font-bold">
									?
								</span>
							)}
							<ChevronDownMini
