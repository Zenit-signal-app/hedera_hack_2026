...
# ============ Post-Deploy: Auto-Association ============

header "Configuring Auto-Associations"

# Check if we have a contract address
if [ -n "$CONTRACT_ADDRESS" ] && [ "$DEPLOY_STATUS" = "success" ]; then
    info "Setting max automatic token associations to -1 (unlimited)..."
    
    # Create a temporary JS script to update the contract
    UPDATE_SCRIPT=$(mktemp /tmp/update-contract-XXXXXX.js)
    
    cat > "$UPDATE_SCRIPT" <<'JSEOF'
const { Client, ContractUpdateTransaction, PrivateKey } = require('@hashgraph/sdk');
const fs = require('fs');
...
if (!operatorId || !operatorKeyStr) {
    console.error('OPERATOR_ID and OPERATOR_KEY required');
    process.exit(1);
}
...
function getClient(network) {
    let client;
    switch (network) {
        case 'hedera_testnet': client = Client.forTestnet(); break;
        case 'hedera_mainnet': client = Client.forMainnet(); break;
        case 'hedera_local': client = Client.forLocalNode(); break;
...
async function main() {
    ...
    if (receipt.status.toString() !== 'SUCCESS') {
        console.error('Update failed:', receipt.status.toString());
        client.close();
        process.exit(1);
    }
    
    console.log('Auto-associations set to unlimited (-1)');
    client.close();
}
...
JSEOF
    ...
    # Try to update auto-associations
    if command -v node &>/dev/null; then
        set +e
        UPDATE_OUTPUT=$(node "$UPDATE_SCRIPT" "$NETWORK" "$CONTRACT_ADDRESS" 2>&1)
        UPDATE_EXIT=$?
        set -e
        rm -f "$UPDATE_SCRIPT"
        
        if [ $UPDATE_EXIT -eq 0 ]; then
            ok "Auto-associations configured successfully"
        else
            warn "Could not set auto-associations: $UPDATE_OUTPUT"
            warn "You may need to call ContractUpdateTransaction manually"
        fi
    else
        warn "Node.js not available - skipping auto-association update"
        warn "You will need to call ContractUpdateTransaction manually"
    fi
else
    if [ -z "$CONTRACT_ADDRESS" ]; then
        warn "No contract address found - skipping auto-association"
    else
        warn "Deployment not successful - skipping auto-association"
    fi
fi
